...
export const card = (() => {
  ...
  const lists = new Map([
    ...
    [
      '```',
      `<code class="font-monospace text-${theme.isDarkMode(
        'light',
        'dark'
      )}">$1</code>`,
    ],
  ...
  const renderButton = (comment) => {
    return `
        <div class="d-flex flex-wrap justify-content-between align-items-center" id="button-${
          comment.uuid
        }">
            <div class="d-flex flex-wrap justify-content-start align-items-center">
                ${
                  ...
                  config.get('can_reply') === undefined
                    ? `<button style="font-size: 0.8rem;" onclick="comment.reply(this)" data-uuid="${
                        comment.uuid
                      }" class="btn btn-sm btn-outline-${theme.isDarkMode(
                        'light',
                        'dark'
                      )} rounded-3 py-0 me-1">Reply</button>`
                    : ''
                }
                ${
                  owns.has(comment.uuid) &&
                  (config.get('can_edit') == true ||
                    config.get('can_edit') === undefined)
                    ? `<button style="font-size: 0.8rem;" onclick="comment.edit(this)" data-uuid="${
                        comment.uuid
                      }" class="btn btn-sm btn-outline-${theme.isDarkMode(
                        'light',
                        'dark'
                      )} rounded-3 py-0 me-1">Edit</button>`
                    : ''
                }

                ${
                  session.get('token').split('.').length === 3
                    ? `<button style="font-size: 0.8rem;" onclick="comment.remove(this)" data-uuid="${
                        comment.uuid
                      }" data-own="${
                        comment.own
                      }" class="btn btn-sm btn-outline-${theme.isDarkMode(
      ...
      'dark'
                    ...
                    <div class="d-flex justify-content-start align-items-center">
                        <p class="my-0 mx-1" data-count-like="${
                          comment.like.love
                        }">${comment.like.love} like</p>
                        <i class="py-1 me-1 px-0 pb-0 ${
                          likes.has(comment.uuid)
  ...
  const renderTracker = (comment) => {
    ...
    return `
        ...
        <p class="text-${theme.isDarkMode(
          'light',
          'dark'
        )} mb-1 mx-0 p-0" style="font-size: 0.7rem;" id="ip-${
      comment.uuid
    }"><i class="fa-solid fa-location-dot me-1"></i>${
      util.escapeHtml(comment.ip) +
      (tracker.has(comment.ip)
        ? ' ' + tracker.get(comment.ip)
        : `<span class="ms-2 mb-1 placeholder col-2 rounded-3"></span>`)
    }</p>
        <p class="text-${theme.isDarkMode(
          'light',
          'dark'
        )} m-0 p-0" style="font-size: 0.7rem;"><i class="fa-solid fa-mobile-screen-button me-1"></i>${util.escapeHtml(
      comment.user_agent
    )}</p>
        </div>`;
  };
  ...
  const renderHeader = (is_parent) => {
    if (is_parent) {
      return `class="card-body bg-theme-${theme.isDarkMode(
        'dark',
        'light'
      )} shadow p-3 mx-0 mt-0 mb-3 rounded-4" data-parent="true"`;
    }

    return `class="card-body border-start bg-theme-${theme.isDarkMode(
      'dark',
      'light'
    )} py-2 ps-2 pe-0 my-2 ms-2 me-0"`;
  };
    ...
    return `
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            <p class="text-${theme.isDarkMode(
              'light',
              'dark'
            )} text-truncate m-0 p-0" style="font-size: 0.95rem;">${renderTitle(
      comment,
      is_parent
    )}</p>
            <small class="text-${theme.isDarkMode(
              'light',
              'dark'
            )} m-0 p-0" style="font-size: 0.75rem;">${
      comment.created_at
    }</small>
        </div>
        <hr class="text-${theme.isDarkMode('light', 'dark')} my-1">
        <p class="text-${theme.isDarkMode(
          'light',
          'dark'
        )} mt-0 mb-1 mx-0 p-0" style="white-space: pre-wrap !important" id="content-${
      comment.uuid
    ...
    fetch(`https://ipapi.co/${comment.ip}/city`)
      ...
      .then((res) => {
        ...
        document.getElementById(
          `ip-${comment.uuid}`
        ).innerHTML = `<i class="fa-solid fa-location-dot me-1"></i>${
          util.escapeHtml(comment.ip) + ' ' + res
        }`;
      })
